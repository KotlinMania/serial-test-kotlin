...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `SerialTest.swiftmodule/`
...
//      `import SerialTest` above would fail at compile time.
//
//   2. The static archive `libSerialTest.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "SerialTest" }` and
//      `flattenPackage = "io.github.kotlinmania.serialtest"` configuration in
...
//      via the `SerialTestLibrary` product.
//
...
final class SerialTestExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "SerialTest swift module imported cleanly")
    }
}
