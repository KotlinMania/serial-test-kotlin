# Claude Code Project Instructions — serial-test-kotlin

## Project Overview

This is **serial-test-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`serial_test`](https://crates.io/crates/serial_test).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/serial_test/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/serial_test>
